...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!scope) return
  if (!scope.allRoadmaps && scope.roadmapIds.length === 0) {
    return res.status(200).json({
      categories: [],
      topics: [],
      levels: MODULE_LEVELS.map(key => ({ key, roadmap_count: 0 })),
  ...
  const placeholders = scope.roadmapIds.map(() => '?').join(', ')
  const roadmapRestriction = scope.allRoadmaps ? '' : `WHERE roadmaps.id IN (${placeholders})`
  const topicRestriction = scope.allRoadmaps ? '' : `WHERE roadmap_topics.roadmap_id IN (${placeholders})`
  const moduleRestriction = scope.allRoadmaps ? '' : `AND roadmap_id IN (${placeholders})`
  const scopeParams = scope.allRoadmaps ? [] : scope.roadmapIds
  ...
  const [categories, topics, duration] = await Promise.all([
    ...
    db.get(
      `SELECT MIN(duration_weeks_min) AS min_weeks,
       ...
       FROM roadmaps ${roadmapRestriction}`,
      scopeParams
    )
  ])
  ...
  const levelCounts = await db.all(
    `SELECT level AS key, COUNT(DISTINCT roadmap_id) AS roadmap_count
     FROM modules WHERE level IS NOT NULL ${moduleRestriction} GROUP BY level`,
    scopeParams
  )
  ...
  return res.status(200).json({
    ...
    unclassified_roadmaps: (await db.get(
      `SELECT COUNT(*) AS count FROM roadmaps
       WHERE category_id IS NULL ${scope.allRoadmaps ? '' : `AND id IN (${placeholders})`}`,
      scopeParams
    )).count
  })
}
